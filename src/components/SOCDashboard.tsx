...
import { AlertTriangle, Shield, Car, Activity, Search, Filter, Wifi, Zap } from "lucide-react";
...
export const SOCDashboard = () => {
  ...
  const [fleetStatusFilter, setFleetStatusFilter] = React.useState<string | null>(null);
  const [fleetTimeFilter, setFleetTimeFilter] = React.useState<string | null>(null);
  ...
  const filterFleetByTime = (vehicle: any) => {
  ...
  const filteredVehicles = vehicles
    ?.filter(v => (fleetStatusFilter ? v.tee_status === fleetStatusFilter : true))
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-10 left-10 w-2 h-2 bg-primary/20 rounded-full animate-pulse" style={{animation: 'float 4s ease-in-out infinite'}}></div>
        <div className="absolute top-32 right-20 w-1 h-1 bg-success/30 rounded-full animate-ping" style={{animationDelay: '1s'}}></div>
        <div className="absolute bottom-40 left-1/4 w-1.5 h-1.5 bg-warning/25 rounded-full animate-pulse" style={{animation: 'float 6s ease-in-out infinite', animationDelay: '2s'}}></div>
        <div className="absolute top-1/2 right-10 w-1 h-1 bg-info/40 rounded-full animate-ping" style={{animationDelay: '3s'}}></div>
        <div className="absolute bottom-20 right-1/3 w-2 h-2 bg-primary/15 rounded-full animate-pulse" style={{animation: 'float 5s ease-in-out infinite', animationDelay: '1s'}}></div>
        <div className="absolute top-1/4 left-1/2 w-1 h-1 bg-critical/20 rounded-full animate-ping" style={{animationDelay: '4s'}}></div>
        <div className="absolute bottom-32 left-10 w-1.5 h-1.5 bg-success/25 rounded-full" style={{animation: 'float 7s ease-in-out infinite', animationDelay: '2.5s'}}></div>
      </div>
      
      {/* Floating Network Icons */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <Wifi className="absolute top-16 right-1/4 h-4 w-4 text-primary/10 animate-bounce" style={{animationDelay: '0.5s', animationDuration: '3s'}} />
        <Zap className="absolute bottom-32 left-16 h-3 w-3 text-warning/15 animate-bounce" style={{animationDelay: '1.5s', animationDuration: '4s'}} />
        <Shield className="absolute top-1/3 left-1/3 h-3 w-3 text-success/12 animate-bounce" style={{animationDelay: '2.5s', animationDuration: '3.5s'}} />
      </div>
      ...
      <div className="flex items-center justify-between relative z-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button variant="outline" size="sm" className="hover:scale-105 transition-transform duration-200 hover:border-primary/50 hover:bg-primary/5">
                    ...
                    <div className="absolute right-0 mt-2 w-64 bg-popover border rounded shadow-lg z-10 p-4">
                      <div className="mb-4">
                        ...
                        <select
                          ...
                          onChange={e => setFleetStatusFilter(e.target.value || null)}
                        >
                          <option value="">All</option>
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={e => setFleetTimeFilter(e.target.value || null)}
                        >
